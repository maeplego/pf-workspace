...
import { SearchHitRow, type Hit } from "../../../components/SearchHitRow";
    ...
    <div className="shell">
      ...
      {q && !error ? (
        <ul style={{ listStyle: "none", padding: 0 }}>
          {hits.length === 0 ? (
            <li className="muted">ヒットなし（Wiki ページやボードを作成してから検索してください）</li>
          ) : null}
          {hits.map((h) => (
            <SearchHitRow key={`${h.type}-${h.id}`} hit={h} query={q} href={hitHref(workspaceId, h, devUser)} />
          ))}
        </ul>
      ) : null}
    </div>
